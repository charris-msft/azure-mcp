...
public sealed class SearchService(ISubscriptionService subscriptionService, ICacheService cacheService) : BaseAzureService, ISearchService
{
    ...
    private async Task<SearchIndexClient> GetSearchIndexClient(string serviceName, RetryPolicyArguments? retryPolicy)
    {
        ...
        var searchClient = await _cacheService.GetAsync<SearchIndexClient>(CACHE_GROUP, key, s_cacheDurationClients);
        if (searchClient == null)
        {
            ...
            await _cacheService.SetAsync(CACHE_GROUP, key, searchClient, s_cacheDurationClients);
        }
        return searchClient;
    }
